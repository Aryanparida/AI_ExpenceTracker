...
const generateTransactions = (catMap) => {
    ...
    for (let monthsAgo = 11; monthsAgo >= 0; monthsAgo--) {
        ...
        if (rng() < 0.5) {
            add(rangeInt(8, 25), 'Other Income', rangeFloat(15, 120), 'income', pick(['Cashback', 'Refund', /* possibly more items, truncated in screenshot */]));
        }
        ...
        for (let day = 1; day <= monthLastDay; day++) {
            ...
            if (!isWeekend && rng() < 0.8) {
                add(day, 'Food & Dining', rangeFloat(4, 8), 'expense', pick(['Morning coffee', 'Coffee', 'Latte'] /* truncated */));
            }
            if (!isWeekend && rng() < 0.55) {
                add(day, 'Food & Dining', rangeFloat(10, 18), 'expense', pick(['Lunch', 'Salad bowl', 'Sandwich'] /* truncated */));
            }
            if (isWeekend && rng() < 0.5) {
                add(day, 'Food & Dining', rangeFloat(28, 75), 'expense', pick(['Dinner out', 'Restaurant', 'Brunch'] /* truncated */));
            }
